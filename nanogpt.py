...
class GPT(nn.Module):
    ...
    def crop_seq_length(self, seq_length):
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0, top_k=None):
        ...
        for _ in range(max_new_tokens):
            # if the sequence context is growing too long we must crop it at seq_length
            idx_cond = idx if idx.size(1) <= self.config.seq_length else idx[:, -self.config.seq_length:]
